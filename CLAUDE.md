...
Test suite: **263 unit tests, <1s** (`cd backend && uv run pytest -v`) + **14 Postgres integration tests** (skipped unless `TEST_DATABASE_URL` points at a scratch DB; CI runs them). Run after engine, config, tax, or scenario changes.
