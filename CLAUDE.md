...
- **Reconnecting: `RequireMFAException` does not mean what it says.** `monarchmoney`'s `_multi_factor_authenticate` raises `RequireMFAException("Multi-Factor Auth Required")` for *any* HTTP 403 and swallows body-parse failures (`except Exception: pass`), so a stale TOTP, a CAPTCHA wall (`error_code: CAPTCHA_REQUIRED`, recoverable via the lib's `login_with_cookies()`), and a blocked client all produce the identical message. Try a fresh code on a new 30-second cycle first — that was the fix on Sept 15 2026. To see the real cause, POST to `https://api.monarch.com/auth/login/` directly and print the status + body. Note `monarch_login.py` now uses `getpass`, so password and MFA entry do not echo — it is not frozen.
- **A Monarch 401 is terminal, never retried.** The session token is an opaque server-side key with no client-readable expiry, so a revocation is only detectable by calling. `MonarchClient.connect()` therefore probes with `get_subscription_details` (cheapest authenticated query) and `_typed_errors` maps 401 -> `MonarchAuthError` / 429 -> `MonarchRateLimitError` on every client method. `run_full_sync` re-raises `MonarchAuthError` past its per-step handlers, `sync_balance_snapshots` stops its per-account loop (partial rows still upserted), and the Celery task skips its retry. This is the Sept 2026 outage: a token revoked ~Sept 6 reported itself as **429 Too Many Requests** because a dead session was swallowed by every handler, fanned out to ~30 rejected calls per pass, and retried 3x — we manufactured our own rate limit and it hid the real 401. Never let an auth failure re-enter the retry path.
...
- Drawdown waterfall order: taxable → IRA-B (after 59½) → forced RMDs → Roth. A pool is drawable whenever it holds money — `property_sales` gates ONLY sale mechanics, never the waterfall (fire-master#12). `custom_assumptions.roth_pool` = `{starting_balance, return_rate}` is the ONLY door tax-free money enters the projection (`tax_free_reserve` accounts feed net worth only); drawn last, no age gate, no RMDs (fire-master#13). RMDs: from `config.rmd_start_age` the IRS Uniform Lifetime minimum is forced from IRA-A+IRA-B (same `_get_rmd_divisor` as the tax engine); the excess beyond the open gap is redeposited to taxable, carried in `WealthPoolPoint.rmd_redeposit` inside `ira_draw` — never add it to cash. `projection.enforce_rmd: false` opts out (fire-master#14). Full mechanics: ARCHITECTURE.md.
- Cash can go negative ONLY while no drawable pool exists (pre-rescue bridge stress; frontend clamps display at 0). Once the taxable (then Roth) pool is funded, a repair draw tops cash back to exactly $0 — a resting-at-zero cash line means "living off pool draws, no buffer."
- One-off cashflow events dated before today are dropped (not clamped to month 0).
- **Cashflow events reach EVERY forward engine** via `build_cashflow_schedule()` in `fire_projections.py` — `project_wealth_pools`, `project_lifetime` (→ `/timeline` `on_track`), and Monte Carlo all consume it; `compute_bridge_status`/`compute_readiness` use `_recurring_events_active_now()` for the "now" snapshot (fire-master#16/#17: three of those paths were event-blind and simulated a zero-income household for an event-based plan). Never expand events inline in an engine again. Rules: calendar-month offsets, monthly/quarterly/annual recurrence in phase with the start date, end_date month inclusive, probability-weighted. Single-pool engines (`project_lifetime`, MC) SKIP conversion INCOME events (sale proceeds / vests matched by `sell_event_label_match`, `illiquid_vest_event_match`, `property_sales[].suppress_cashflow_match`) — their net worth already holds the asset at book value. `project_lifetime` falls back to trailing-12-month transaction income ONLY when neither income sources nor income events exist.
- **Social Security has ONE setting** (fire-master#20, Sept 16 2026): `social_security_monthly` is the benefit payable at `social_security_start_age`, read identically by `project_wealth_pools`, `project_lifetime`, Monte Carlo, readiness and `compute_fire_number`. The legacy `projection.ss_claim_age` / `ss_early_reduction` keys are ignored by the engine and deleted by the config page on save (explicit `null`); they re-reduced an amount already entered for its claiming age and let the pool engine disagree with every other one. To model early claiming, enter the reduced amount and the early age in the main fields.
- Monte Carlo (`monte_carlo.py`) shares the real-terms frame: lognormal nominal returns + correlated stochastic inflation (ρ=−0.25) → real return per year; spending/income/SS flat real. Vol/correlation overrides live in `custom_assumptions.monte_carlo`. **Opt-in** `monte_carlo.simulation_method: "historical_blocks"` swaps the iid draw for overlapping 7-year blocks of joint (S&P, 10y Treasury, CPI) 1928-2025 observations (`app/data/historical_market_returns.py`, Damodaran, cited), log-recentered to the configured return/inflation so only the sequences are historical; `historical_block_years`, `stock_weight` tune it. Default stays parametric — never flip it for everyone. Do NOT reintroduce nominal `(1+i)**yr` multipliers on flows — that was the pre-Jul-2026 double-count bug. Known limit: ONE undifferentiated portfolio (no 59½ gate / taxable-first waterfall / SEPP), so it cannot measure sequence risk inside a bridge — a pool-aware MC is separate work.
