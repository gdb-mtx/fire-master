...
Test suite: **192 tests, <1s** (`cd backend && uv run pytest -v`). Run after engine, config, tax, or scenario changes.
...
- The config page MERGES the tax/projection blocks with their existing contents on save — keys the form doesn't manage (e.g. the matchers above) survive. Keep it that way.
